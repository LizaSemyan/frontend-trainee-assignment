...
export const AdsCard = ({ ad }: { ad: Advertisement }) => {
  return (
    ...
    >
      <CardMedia
        component="img"
        image={ad.images?.[0] ?? 'https://via.placeholder.com/100'}
        alt={ad.title}
        sx={{
          width: 100,
          height: 100,
      ...
      <Box sx={{ flex: 1 }}>
        ...
        <Box sx={{ mt: 1, display: 'flex', gap: 1 }}>
          <Chip size="small" label={ad.status} />
          <Chip size="small" label={ad.priority} />
        </Box>
      </Box>

      <Box>
        <Button variant="contained" color="success" component={Link} to={`/item/${ad.id}`}>
          Открыть →
        </Button>
      </Box>
    </Card>
